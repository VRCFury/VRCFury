        ...
        public void Apply() {
            ...
            foreach (var c in avatarObject.GetComponentsInChildren<OGBPenetrator>(true)) {
                ...
                if (c.configureTps) {
                    ...
                    var renderersWithTps = renderersToCheck
                        .Where(r => r.sharedMaterials.Any(m => OGBPenetratorSizeDetector.IsTps(m)))
                        .ToArray();

                    if (renderersWithTps.Length == 0) {
                    ...
                    foreach (var renderer_ in renderersWithTps) {
                        ...
                        foreach (var matSlot in Enumerable.Range(0, skin.sharedMaterials.Length)) {
                            ...
                            if (!OGBPenetratorSizeDetector.IsTps(mat)) continue;
